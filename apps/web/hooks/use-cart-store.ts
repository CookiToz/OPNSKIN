    ...
    try {
      ...
      if (Array.isArray(data.cart)) {
        const mappedItems = data.cart.map((item: any) => ({
          id: item.offerId,
          name: item.offer?.itemName || item.offer?.itemId || 'Skin inconnu',
          price: item.offer?.price || 0,
          image: item.offer?.itemImage || '/placeholder.svg',
        }));
        set((state) => ({ ...state, items: mappedItems }));
      }
    } catch (e) {
      console.error('Erreur syncWithBackend:', e);
    }
  },
})); 
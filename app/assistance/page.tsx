    ...
    <div className="min-h-screen flex flex-col items-center justify-start bg-opnskin-bg-primary py-10 px-4">
      <Card className="w-full max-w-2xl mb-8 bg-opnskin-bg-card border-opnskin-bg-secondary">
        <CardContent className="py-6">
          <h1 className="text-3xl font-bold font-rajdhani text-opnskin-primary mb-6">{t('assistance.title', 'Assistance & FAQ')}</h1>
      ...
      {showForm && !sent && (
        <Card className="w-full max-w-2xl bg-opnskin-bg-card border-opnskin-bg-secondary mb-8">
          <CardContent className="py-6">
            <h2 className="text-xl font-bold font-rajdhani text-opnskin-primary mb-4">{t('assistance.form_title', 'Formulaire de contact')}</h2>
            <form onSubmit={handleSend} className="space-y-4">
              <div>
                <label className="block text-sm text-opnskin-text-secondary mb-1">{t('assistance.form_subject', 'Objet')}</label>
                <Input
                  type="text"
              ...
              <div>
                <label className="block text-sm text-opnskin-text-secondary mb-1">{t('assistance.form_message', 'Votre message')}</label>
                <Textarea
              ...
              <div>
                <label className="block text-sm text-opnskin-text-secondary mb-1">{t('assistance.form_email', 'Votre email')}</label>
                <Input
                  type="email"
      ...
      {sent && (
        <Card className="w-full max-w-2xl bg-opnskin-bg-card border-opnskin-bg-secondary">
          <CardContent className="py-8 flex flex-col items-center">
            <Badge className="bg-opnskin-accent/10 text-opnskin-accent border-opnskin-accent/30 mb-4 text-lg">{t('assistance.form_sent_badge', 'Message envoyé !')}</Badge>
            <div className="text-opnskin-text-primary text-lg mb-2">{t('assistance.form_sent_thanks', 'Merci pour votre message.')}</div>
            <div className="text-opnskin-text-secondary text-sm text-center">{t('assistance.form_sent_info', "Notre équipe vous répondra sous 24h-48h à l'adresse indiquée.")}</div>
          </CardContent>
        </Card>
      )}
    </div>
